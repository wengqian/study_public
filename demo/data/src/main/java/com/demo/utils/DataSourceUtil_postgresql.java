 ...
 */
//@Component("dataPool")
public class DataSourceUtil_postgresql {
    //基础数据源
    public static String baseDataSource  = "baseJdbcTemplate";
    //All_数据源
    public static Map<String,DruidDataSource> all_dataPool;

    public static TreeMap<String,DruidDataSource> select_dataPool;

    public static JdbcTemplate baseJdbcTemplate ;

    public static final int select_index = 0;//为select_dataPoll的总数

    public static  JdbcTemplate getJdbcTemplate(String dataSorcename)  {
        if(all_dataPool==null){
            DataSourceUtil_postgresql.initDataPool();
        }
        if(baseDataSource.equals(dataSorcename)){//基础数据库
            ...
            if(isSelectTrue_postgresql(baseJdbcTemplate)){
                return baseJdbcTemplate;
            }
        }
        //配置的数据库
        JdbcTemplate jdbcTemplate ;
        String all_datasorce_name = "";
        //下面是更具map的迭代不是有序列的,看RP查询
        while(true){
            if(dataSorcename.isEmpty()){
                System.out.print("所有的数据库查询异常,请管理员尽快排查!");
                //抛出异常_所有数据库的连接都出了问题
                break;
            }
            //数据库不是关闭的
            if(!select_dataPool.get(dataSorcename).isClosed()){
                jdbcTemplate = new JdbcTemplate(select_dataPool.get(dataSorcename));
                if(isSelectTrue_postgresql(jdbcTemplate)){
                    return jdbcTemplate;
                }
                //通知管理员.某个数据库连接出现问题
                all_datasorce_name+=","+dataSorcename;
                System.out.println("数据源为"+dataSorcename+"的数据库连接异常,请管理员尽快排查");
            }
            //从备份所有的从库中进行可以搜索
            //查询负载均衡的机制需要写一下
            for(String key :select_dataPool.keySet()){//根据map的迭代出现不是有序的
                if(all_datasorce_name.indexOf(dataSorcename) == -1){
                    dataSorcename = key;
                    break;
                }
            }
            ...
            return  null;
//        return new JdbcTemplate(all_dataPool.get(dataSorcename));
    }
    //postgresql 校验是否可以查询数据
    public static boolean isSelectTrue_postgresql (JdbcTemplate jdbcTemplate){
        try{
            jdbcTemplate.queryForObject("select count(*)",Long.class);
        }catch (Exception e){
            e.printStackTrace();
            return false;
        }
    ...
    public static  DruidDataSource getDataSource1(String dataSorcename)  {
        ...
        return all_dataPool.get(dataSorcename);
    }
    /**
     * 初始化数据池
     * */
    public static void initDataPool()  {
        all_dataPool = new HashMap<String, DruidDataSource>();
        select_dataPool = new TreeMap<String,DruidDataSource>();
//        all_dataPool.put("baseJdbcTemplate", (JdbcTemplate)SpringBeanUtil.getBean("baseJdbcTemplate"));
    }
    /**
     * 创建新的数据源连接,并且让如到数据池中
     * */
...
//            all_dataPool.put(result.getString("datasource_name"),jdbcTemplate1);
//
